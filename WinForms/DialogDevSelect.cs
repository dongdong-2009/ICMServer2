...
namespace ICMServer
{
    public partial class DialogDevSelect : Form
    {
        private List<Device> m_Devices = new List<Device>();

        public DialogDevSelect()
        {
            InitializeComponent();
        }

        private void DialogDevSelect_Load(object sender, EventArgs e)
        {
            using (var db = new ICMDBContext())
            {
                var entranceDevices = (from d in db.Devices
                                       where d.type != 0 && d.type != 8 && d.type != 10
                                       select d).ToList();
