    ...
    static class Eextentions
    {
        ...
        public static bool AreObjectsEqual(object objectA, object objectB, params string[] ignoreList)
        {
            ...
            if (objectA != null && objectB != null)
            {
                ...
                // if objectA implements IEquatable<T> interface, call objectA.Equals(objectB)
                if (objectType.GetInterfaces().Any(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IEquatable<>)))
                {
                    return Convert.ToBoolean(objectType.GetMethod("Equals", new Type[] { objectType }).Invoke(objectA, new object[] { objectB }));
                }
                ...
                foreach (PropertyInfo propertyInfo in objectType.GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(p => p.CanRead && !ignoreList.Contains(p.Name)))
                {
                    ...
                    // To avoid TargetParameterCountException 
