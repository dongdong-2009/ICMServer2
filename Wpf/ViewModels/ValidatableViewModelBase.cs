...
{
    /// <summary>
    /// Base for bservableObjects which require property validation
    /// </summary>
    ...
    public class ValidatableViewModelBase : ViewModelBase, INotifyDataErrorInfo
    {
        ...
        //protected async void SetAndValidatePropertyAsync<T>(ref T field, T newValue = default(T), bool broadcast = false, [CallerMemberName] string propertyName = null)
        //{
        //    if (Set<T>(ref field, newValue, broadcast, propertyName))
        //    {
        ...
        /// <summary>
        /// Initialises a new instance of the <see cref="ValidatableViewModelBase"/> class, without using an <see cref="IModelValidator"/>
        /// </summary>
        ...
        /// <summary>
        /// Initialises a new instance of the <see cref="ValidatableViewModelBase"/> class, using the specifies <see cref="IModelValidator"/>
        /// </summary>
        /// <param name="validator">Validator adapter to use to perform validations</param>
        public ValidatableViewModelBase(IModelValidator validator) : this()
        {
        ...
        protected virtual async Task<bool> ValidateAsync()
        {
            ...
            var results = await this.Validator.ValidateAllPropertiesAsync().ConfigureAwait(false);  // 呼叫驗證器的非同步驗證
            if (results == null)
        ...
        //ActionBlock<string> validationTasks = new Action<string>();
        protected TaskQueue validationTasks = new TaskQueue();
        ...
        public override async void RaisePropertyChanged(string propertyName)
        {
            ...
            //DebugLog.TraceMessage(propertyName);
            if (this.Validator != null && this.AutoValidate && propertyName != "HasErrors")
            {
                //await validationTasks.Enqueue<bool>(() => this.ValidatePropertyAsync(propertyName));
